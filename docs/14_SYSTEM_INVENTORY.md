...
| Cloud saves | ✅ M15 | Supabase-backed, opt-in only; `SaveManager` sync/conflict resolution, RLS-gated `player_saves` table. See `docs/05_CURRENT_SYSTEMS.md`'s M15 section |
| Account sign-in (email/password + Google) | 🟡 M15 | Email/password shipped (`AuthManager.gd`, `SettingsMenu` Account tab); **Google Sign-In deferred** — Godot 4.3 has no native Android deep-link API and M13 hasn't produced a working export yet, so the plugin work has no real Android target to build against. Optional/opt-in either way, never required to play |
